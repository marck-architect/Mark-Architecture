...
export async function POST(request: NextRequest) {
  try {
    ...
    // 3. Complete Portfolio Projects (8 items)
    ...
    // 4. Complete Services & Tiers Catalog (7 Services)
      ...
      for (const s of servicesCatalog) {
        ...
        if (sData && s.tiers) {
          for (let tIdx = 0; tIdx < s.tiers.length; tIdx++) {
            ...
            if (tData) {
              const tierObj = t as Record<string, any>;
              if (tierObj.price_pkr) {
                await supabase.from("pricing_rules").insert({
                  tier_id: tData.id,
                  plot_size: "Any",
                  price_pkr: tierObj.price_pkr,
                });
              } else if (tierObj.pricing) {
                for (const [plot, price] of Object.entries(tierObj.pricing)) {
                  await supabase.from("pricing_rules").insert({
                    tier_id: tData.id,
                    plot_size: plot,
    ...
    // 5. Complete Collection Packages (5 Packages)
    ...
    // 6. Complete Leadership & Team (2 Members)
    const teamInserts = [
      {
        id: "team_1",
        ...
        specialization:
          "Minimalist Residential Architecture, Passive Solar Layouts & Municipal Codes",
        photo_url: getStorageUrl(
    ...
    const faqInserts = [
      ...
      },
      {
        ...
        category: "consultation",
        question:
          "How far in advance must I book an architectural consultation?",
        answer:
    ...
    // 8. Complete Testimonials (2 Items)
    ...
    try {
      await supabase.from("testimonials").upsert(rawTestimonials, {
    ...
    // 9. Complete Achievements & Locations
    ...
    // 10. Seed Universal Site Content
    try {
      await supabase.from("site_content").upsert(
        [
          {
            section_key: "projects",
            content: projectInserts,
          },
          {
            section_key: "collection_packages",
            content: collectionInserts,
          },
          {
    ...
    return NextResponse.json({
      ...
      seeded: {
        ...
        testimonials: rawTestimonials.length,
      },
