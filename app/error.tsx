...
export default function Error({
  ...
  useEffect(() => {
    console.error("Unhandled error captured by app/error.tsx:", error);
  }, [error]);
    ...
    <div className="min-h-screen bg-[#FCF8F8] text-[#1C1B1B] flex flex-col items-center justify-center px-4 py-16 selection:bg-amber-100 selection:text-amber-900">
      ...
      <div className="relative max-w-md w-full bg-white border border-stone-200 rounded-2xl shadow-xl shadow-stone-200/50 p-8 md:p-10 text-center z-10">
        ...
        <p className="text-xs text-stone-600 mb-6 leading-relaxed">
          An unexpected error occurred while rendering this space. You may try
          reloading the current page or return to the atelier homepage.
        </p>

        {error.digest && (
          <p className="text-[11px] font-mono text-stone-500 bg-stone-50 border border-stone-200/80 rounded-lg py-1.5 px-3 mb-6 truncate">
            Digest: {error.digest}
          </p>
        )}
