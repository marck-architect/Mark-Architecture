...
export const CartDrawer: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = React.useState<"cart" | "orders">("cart");
  ...
  // If cart is empty but user has confirmed orders, default to the orders view
  React.useEffect(() => {
  ...
  }, [cart.length, confirmedOrders.length]);
        ...
        <div className="fixed inset-0 z-[80] overflow-hidden">
          ...
          <div className="absolute inset-y-0 right-0 max-w-full min-w-0 flex">
            ...
            >
              ...
              {/* Tab Navigation (visible when not in checkout step) */}
              {step === "cart" && (
                <div className="flex border-b border-outline-variant/30 px-3 sm:px-6 bg-surface-container-lowest dark:bg-zinc-900/60 shrink-0">
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    className={cn(
                      "py-3 px-3 text-xs font-semibold border-b-2 transition-all flex items-center gap-1.5 cursor-pointer ml-1",
                      activeTab === "orders"
                        ? "border-tertiary text-tertiary font-bold"
                        : "border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300",
                    )}
                  >
                    ...
                    {confirmedOrders.length > 0 && (
                      <span className="bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 text-[10px] font-bold px-1.5 py-0.2 rounded-full border border-emerald-500/30">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between pb-2 border-b border-outline-variant/30">
                      <div>
                        <h4 className="font-playfair text-sm font-bold text-secondary dark:text-zinc-200">
                          Saved Orders &amp; Receipts
                        </h4>
                        <p className="text-[10px] text-zinc-500 dark:text-zinc-400">
                          Persisted on this device until you clear it.
                        </p>
                      </div>
                      {confirmedOrders.length > 0 && (
                        <button
                          ...
                          onClick={() => {
                            if (
                              window.confirm(
                                "Are you sure you want to clear your saved orders and bookings from this device?",
                              )
                            ) {
                              clearConfirmedOrders();
                            }
                          }}
                          className="text-[11px] text-zinc-400 hover:text-red-500 transition-colors flex items-center gap-1 cursor-pointer font-medium p-1 rounded hover:bg-red-500/10"
                        >
                    ...
                    {confirmedOrders.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-12 text-center space-y-3">
                        <div className="w-12 h-12 rounded-full bg-surface-container dark:bg-zinc-900 flex items-center justify-center text-zinc-400">
                          <Receipt className="w-6 h-6" />
                        </div>
                        <p className="font-playfair text-base font-semibold text-zinc-800 dark:text-zinc-200">
                          No confirmed orders yet
                        </p>
                        <p className="text-xs text-zinc-500 max-w-xs font-light">
                          Completed consultation bookings and paid service
                          packages will be stored here for your reference.
                        </p>
                        <button
                          type="button"
                          onClick={() => setActiveTab("cart")}
                          className="text-xs font-bold text-tertiary hover:underline pt-2 uppercase tracking-wider cursor-pointer"
                        >
                          View Current Cart
                        </button>
                      </div>
                      ...
                      confirmedOrders.map((order, index) => {
                        const copyRefKey = `ref-${order.id || index}`;
                        const copyTrkKey = `trk-${order.id || index}`;

                        return (
                          <div
                            key={order.id || order.referenceNumber || index}
                            className="p-4 bg-surface-container-low dark:bg-zinc-900/60 border border-outline-variant/30 rounded-2xl space-y-3 relative group"
                          >
                            {/* Card Top: Badges & Remove */}
                            <div className="flex items-start justify-between gap-2 pr-6">
                              <div>
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  ...
                                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-tertiary/10 text-tertiary border border-tertiary/20">
                                    {order.type === "consultation"
                                      ? "Consultation"
                                      : "Service Order"}
                                  </span>
                                </div>
                                <h4 className="font-playfair text-sm font-bold text-secondary dark:text-zinc-100 mt-1.5 leading-snug">
                                  {order.title}
                                </h4>
                              </div>
                              ...
                              <button
                                ...
                                className="absolute top-3 right-3 text-zinc-400 hover:text-red-500 transition-colors p-1 rounded-lg hover:bg-red-500/10 cursor-pointer"
                            ...
                            {/* Price & Timestamp */}
                            <div className="flex items-center justify-between text-xs py-1.5 border-y border-outline-variant/20 font-inter">
                              ...
                              {order.amountPkr > 0 && (
                                <span className="font-montserrat font-bold text-tertiary">
                                  {SafepayService.formatPKR(order.amountPkr)}
                                </span>
                              )}
                            </div>
                            ...
                            {order.type === "consultation" && (
                              <div className="p-3 rounded-xl bg-surface dark:bg-zinc-950/70 border border-outline-variant/25 space-y-2 text-xs">
                                {(order.scheduledDate ||
                                  order.scheduledTime) && (
                                ...
                                {order.meetingUrl ? (
                                  <a
                                    ...
                                    className="w-full mt-1.5 py-2 px-3 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-medium text-xs flex items-center justify-center gap-1.5 transition-colors shadow-sm cursor-pointer"
                                  >
                            ...
                            {order.items && order.items.length > 0 && (
                              <div className="p-2.5 rounded-xl bg-surface dark:bg-zinc-950/70 border border-outline-variant/25 space-y-1.5 text-xs">
                                ...
                                {order.items.map((it, idx) => (
                                  <div
                                    key={idx}
                                    className="flex justify-between text-zinc-600 dark:text-zinc-300"
                                  >
                                    <span>
                                      {it.title}{" "}
                                      {it.quantity > 1
                            ...
                            {/* Client & Reference Identifiers */}
                            <div className="space-y-1 text-[11px] text-zinc-500 dark:text-zinc-400 font-mono pt-1">
                              <div className="flex items-center justify-between">
                                <span>Ref:</span>
                                <div className="flex items-center gap-1.5">
                                  <span className="text-zinc-800 dark:text-zinc-200">
                                    {order.referenceNumber.length > 18
                                      ? `${order.referenceNumber.slice(0, 15)}...`
                              ...
                              {order.tracker && (
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    <span className="text-tertiary">
                                      {order.tracker.length > 18
                                        ? `${order.tracker.slice(0, 15)}...`
                                        : order.tracker}
                                    </span>
                                    <button
                                      type="button"
                                      onClick={() =>
                                        handleCopy(order.tracker!, copyTrkKey)
                                      }
                                      className="p-1 hover:text-tertiary transition-colors cursor-pointer"
                                      title="Copy Tracker Token"
                                    >
                                      {copiedKey === copyTrkKey ? (
                                        <Check className="w-3 h-3 text-emerald-400" />
                                      ) : (
                                        <Copy className="w-3 h-3" />
                                      )}
              ...
              {/* Drawer Footer */}
              {activeTab === "orders" ? (
                <div className="p-4 sm:p-6 border-t border-outline-variant/30 bg-surface-container-low dark:bg-zinc-900 space-y-3">
                  <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                    <span>Stored History</span>
                    <span className="font-semibold text-zinc-700 dark:text-zinc-200">
                      {confirmedOrders.length} saved receipt(s)
                    </span>
                  </div>

                  <a
                    href="tel:+923001234567"
                    className="w-full py-3 px-4 rounded-xl bg-zinc-800 hover:bg-zinc-700 text-zinc-200 text-xs font-semibold flex items-center justify-center gap-2 transition-all cursor-pointer"
                  >
                    <PhoneCall className="w-3.5 h-3.5 text-tertiary" />
                    <span>Studio Support Hotline (+92 300 1234567)</span>
                  </a>

                  <button
                    type="button"
                    onClick={() => setActiveTab("cart")}
                    className="w-full py-3 px-4 rounded-xl bg-tertiary hover:bg-tertiary-hover text-zinc-950 text-xs font-bold flex items-center justify-center gap-2 transition-all cursor-pointer uppercase tracking-wider"
                  >
                    <span>Browse / Edit Cart Packages</span>
